...
export default function NurseMyRequestsPage() {
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        title="طلباتي"
        description="جميع الطلبات التي قبلتها أو أكملتها"
      />
      ...
      <div className="grid grid-cols-2 gap-3">
        <GlassCard variant="nurse" className="p-3 text-center">
          <p className="text-2xl font-bold text-sky-600">
            {toArabicNum(assignments.filter(a => ['assigned', 'accepted', 'in_progress'].includes(a.status)).length)}
          </p>
          <p className="text-xs text-muted-foreground">نشطة</p>
        </GlassCard>
        <GlassCard variant="nurse" className="p-3 text-center">
          <p className="text-2xl font-bold text-green-600">
            {toArabicNum(assignments.filter(a => a.status === 'completed').length)}
          </p>
          ...
          <PullToRefresh onRefresh={handleRefresh}>
            {isLoading ? (
              ...
              <EmptyState
                ...
                description={
                  activeTab === 'active'
                    ? 'ستظهر الطلبات النشطة هنا بعد قبولك للطلبات الجديدة'
                    : 'ستظهر الطلبات المكتملة هنا بعد إنهائها'
                }
              />
            ) : (
              <motion.div
